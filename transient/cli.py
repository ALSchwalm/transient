...
class TransientOptionParser(click.parser.OptionParser):
    ...
    def add_option(self, opts: Iterable[str], dest: str, action: Optional[str] = None,
                   nargs: int = 1, const: Optional[bool] = None,
                   obj: Optional[click.Option] = None) -> None:
...
@click.option('-ssh-timeout', default=90,
...
@click.option('-shutdown-timeout', default=20,
              help='The time to wait for shutdown before terminating QEMU')
@click.option('-shared-folder', '-s', default=[], multiple=True,
